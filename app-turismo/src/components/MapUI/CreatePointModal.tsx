...
export const CreatePointModal: React.FC<CreatePointModalProps> = ({
  ...
  const fetchFaunaTypes = async () => {
    try {
      const url =
        (process.env.EXPO_PUBLIC_BACKEND_URL || 'http://localhost:8081') + '/api/v1/fauna-types';
    ...
    <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
      ...
      <View style={styles.modalContainer}>
        ...
        >
          ...
          <SearchableSelect
            ...
            options={
              formPointType?.toLowerCase() === 'fauna'
                ? [...faunaTypes, { label: 'nuevo +', value: 'nuevo +' }]
                : [
          ...
          <TextInput
            style={styles.input}
            placeholder="Ej. Todo Público"
            placeholderTextColor="#4A5568"
            value={formTargetAudience}
            onChangeText={setFormTargetAudience}
          />

          <Text style={styles.label}>TÍTULO DEL PUNTO</Text>
          <TextInput
            style={styles.input}
            placeholder="Ej. Avistamiento de Lobos Marinos"
            placeholderTextColor="#4A5568"
            value={formTitle}
          ...
          <TextInput
            ...
            placeholder="Escribe detalles del punto..."
            placeholderTextColor="#4A5568"
            multiline
            numberOfLines={3}
          ...
          <Text style={styles.label}>
            {formPointType?.toLowerCase() === 'fauna'
              ? 'IMAGEN / BANNER DE FAUNA (URL)'
              : 'ICONO SVG | IMAGEN (Ruta o URL)'}
          </Text>
          <TextInput
            style={styles.input}
            placeholder={
              formPointType?.toLowerCase() === 'fauna'
                ? 'Ej. https://images.unsplash.com/... (Opcional)'
                : 'Ej. /assets/icon.svg o https://...'
            }
            placeholderTextColor="#4A5568"
            value={formSvgIcon}
          ...
          <TextInput
            style={styles.input}
            placeholder="Ej. Calle Yungay 800, Isla Teja"
            placeholderTextColor="#4A5568"
            value={formAddress}
