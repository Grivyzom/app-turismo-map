...
export default function RegisterScreen({ onRegister, onGoToLogin }: RegisterScreenProps) {
  ...
  // Architecture states
  const [userType, setUserType] = useState<'citizen' | 'partner_owner'>('citizen');
  const [entityType, setEntityType] = useState<'business' | 'creator' | 'media'>('business');
  ...
  return (
    ...
    >
      ...
      >
        <View className="w-full max-w-md self-center mx-5 bg-white/85 backdrop-blur-xl border border-[#c1c8c3]/30 rounded-xl p-8 shadow-2xl shadow-[#1a4335]/10 gap-6">
          <View className="text-center mb-0">
            <Text className="text-[32px] leading-10 tracking-tight font-bold text-[#002d20] mb-2">
              Explora Valdivia
            </Text>
            <Text className="text-base leading-6 text-[#414844]">
              Únete a nuestra red interactiva
            </Text>
          </View>

          {/* Account Mode Switcher */}
          <View className="flex-row bg-[#f3f4f5] border border-[#c1c8c3]/40 p-1 rounded-full">
            <TouchableOpacity
              onPress={() => setUserType('citizen')}
              className={`flex-1 py-2.5 rounded-full items-center ${userType === 'citizen' ? 'bg-white shadow-sm border border-[#c1c8c3]/20' : ''}`}
            >
              <Text
                className={`font-semibold ${userType === 'citizen' ? 'text-[#1a4335]' : 'text-[#717974]'}`}
              >
                Soy Turista
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => setUserType('partner_owner')}
              className={`flex-1 py-2.5 rounded-full items-center ${userType === 'partner_owner' ? 'bg-white shadow-sm border border-[#c1c8c3]/20' : ''}`}
            >
              <Text
                className={`font-semibold ${userType === 'partner_owner' ? 'text-[#1a4335]' : 'text-[#717974]'}`}
              >
                Soy Entidad
              </Text>
            </TouchableOpacity>
          ...
          <View className="gap-4">
            {/* Entity Type Selector (Only for Partners) */}
            {userType === 'partner_owner' && (
              <View className="flex-row justify-between mb-2">
                {(['business', 'creator', 'media'] as const).map((type) => (
                  <TouchableOpacity
                    key={type}
                    onPress={() => setEntityType(type)}
                    className={`px-3 py-2 rounded-xl flex-1 items-center mx-1 border ${entityType === type ? 'bg-[#1a4335] border-[#1a4335]' : 'bg-[#f3f4f5] border-[#c1c8c3]/30'}`}
                  >
                    <Text
                      className={`text-xs font-bold uppercase ${entityType === type ? 'text-white' : 'text-[#717974]'}`}
                    >
                      {type === 'business' ? 'Empresa' : type === 'creator' ? 'Creador' : 'Prensa'}
                    </Text>
                  </TouchableOpacity>
                ))}
              </View>
            ...
            <View className="relative">
              <View className="absolute left-4 top-0 bottom-0 justify-center z-10 pointer-events-none">
                <MaterialIcons
                  name={userType === 'citizen' ? 'person' : 'business'}
                  size={20}
                  color="#717974"
                />
              </View>
              <TextInput
                className="w-full bg-[#f3f4f5] border border-[#c1c8c3]/50 rounded-full px-5 py-3.5 pl-12 text-base text-[#191c1d] shadow-sm"
                placeholder={
                  userType === 'citizen' ? 'Nombre Completo' : 'Nombre de tu Marca / Empresa'
                }
            ...
            {/* Location Input (Only for Citizen) */}
            {userType === 'citizen' && (
              <View className="relative">
                <TouchableOpacity
                  onPress={() => {
                  ...
                  className="w-full bg-[#f3f4f5] border border-[#c1c8c3]/50 rounded-full px-5 py-3.5 pl-12 pr-12 flex-row items-center shadow-sm active:opacity-90"
                >
            ...
            {/* Phone Input (Only for Partner Owner) */}
            {userType === 'partner_owner' && (
              <View className="relative">
                <View className="absolute left-4 top-0 bottom-0 justify-center z-10 pointer-events-none">
                  <MaterialIcons name="phone" size={20} color="#717974" />
                </View>
                <TextInput
                  className="w-full bg-[#f3f4f5] border border-[#c1c8c3]/50 rounded-full px-5 py-3.5 pl-12 text-base text-[#191c1d] shadow-sm"
                  placeholder="Teléfono (Para verificación rápida)"
                  placeholderTextColor="#717974"
            ...
            <View className="relative">
              ...
              <TextInput
                className="w-full bg-[#f3f4f5] border border-[#c1c8c3]/50 rounded-full px-5 py-3.5 pl-12 text-base text-[#191c1d] shadow-sm"
                placeholder={userType === 'citizen' ? 'Correo Electrónico' : 'Email Corporativo'}
                placeholderTextColor="#717974"
            ...
            <View className="relative">
              ...
              <TextInput
                className="w-full bg-[#f3f4f5] border border-[#c1c8c3]/50 rounded-full px-5 py-3.5 pl-12 pr-12 text-base text-[#191c1d] shadow-sm"
            ...
            <TouchableOpacity
              className="w-full mt-2 bg-[#1a4335] rounded-full py-4 shadow-lg shadow-[#1a4335]/20 active:opacity-90"
              onPress={handleRegister}
            >
              <View className="flex-row items-center justify-center gap-2">
                <Text className="text-center text-sm font-semibold tracking-wide text-[#85af9d]">
                  {userType === 'citizen' ? 'Registrarse como Turista' : 'Solicitar Verificación'}
                </Text>
          ...
          <TouchableOpacity className="w-full bg-[#f3f4f5] border border-[#c1c8c3]/40 rounded-full py-3.5 px-5 flex-row items-center justify-center gap-3 shadow-sm active:opacity-90">
      ...
      {/* Citizen Location Modal (kept exactly as it was) */}
      <Modal
